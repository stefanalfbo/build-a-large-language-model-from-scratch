...
def test_token_id_to_embedding_vector_conversion():
    ...
    expected_embedding_weights = torch.tensor(
        [
    ...
    expected_input_id_embeddings = torch.tensor(
        [
            [1.2753, -0.2010, -0.1606],
            [-0.4015, 0.9666, -1.1481],
            [-2.8400, -0.7849, -1.4096],
    ...
    assert torch.allclose(
        embedding_layer.weight.detach(), expected_embedding_weights, atol=1e-4
    )
    ...
    assert torch.allclose(result, expected_input_id_embeddings, atol=1e-4)
